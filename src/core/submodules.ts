...
export interface DiscoverResult {
  ...
  existing: Workspace[];   // 已存在的子模块（不动）
  warnings: string[]; // .gitmodules 里有但跳过的（非 GitHub url、path 不存在等）
}

/**
 * 列出某父 workspace 的所有子模块（DB 中 parent_workspace_id 匹配的行）。
 */
export function listSubmodules(parentWorkspaceId: string): Workspace[] {
  const db = getDb();
  return db
    .query<Workspace, [string]>(
      "SELECT * FROM workspaces WHERE parent_workspace_id = ? ORDER BY submodule_path ASC",
    )
    .all(parentWorkspaceId);
}

/**
 * 解析父 workspace 的 .gitmodules，把发现的子模块同步到 DB。
 *
 * 算法（增量）：
 *   1. 校验 parentWorkspaceId 存在 + 自身不是子模块（不支持嵌套）
 *   2. 解析 .gitmodules 拿 entries
 *   3. 对每个 entry：
 *      - 物理路径不存在 / 非目录 → warnings
 *      - URL 非 GitHub → warnings
 *      - 已存在（按 submodule_path 比对）→ existing
 *      - 否则：pickUniqueAlias → createWorkspace（含 parent_workspace_id）→ added
 ...
 */
export function discoverSubmodules(parentWorkspaceId: string): DiscoverResult {
  const result: DiscoverResult = { added: [], existing: [], warnings: [] };

  const parent = getWorkspaceById(parentWorkspaceId);
  if (!parent) throw new Error(`workspace not found: ${parentWorkspaceId}`);
  ...
  const existingByPath = new Map(existing.map((r: Workspace) => [r.submodule_path, r]));
  ...
  for (const entry of entries) {
    ...
    result.added.push(newWorkspace);
  }

  // existingByPath 还剩的 = DB 有但 .gitmodules 没
  for (const orphan of existingByPath.values()) {
    result.warnings.push(
      `DB 中有子模块 ${orphan.alias}（${orphan.submodule_path}）但 .gitmodules 已无对应条目；不自动删除（避免破坏关联 requirements），需要时请手动 deleteWorkspace`,
    );
  }
...
function pickUniqueAlias(base: string): string {
  ...
  const used = new Set(all.map((r: Workspace) => r.alias));
