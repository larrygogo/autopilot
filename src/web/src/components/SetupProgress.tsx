...
export interface SetupProgressProps {
  current: number;
  /** 步骤标签，长度即步数（默认 2 步：Provider / 代码库） */
  labels?: string[];
}

const DEFAULT_LABELS: string[] = ["Provider", "代码库"];

export function SetupProgress({ current, labels = DEFAULT_LABELS }: SetupProgressProps) {
  const total = labels.length;
  return (
    <ol className="bp-label mb-6 flex items-center gap-2 text-xs">
          ...
          <li key={label} className="flex items-center gap-2">
            <span
              className={cn(
                ...
                isDone && "border-border text-foreground/60",
                !isActive && !isDone && "border-border text-foreground/30",
              )}
            >
              {step}
            </span>
