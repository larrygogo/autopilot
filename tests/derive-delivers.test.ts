 ...
 * 顶层 delivers 不再由用户手填，而是从「哪个阶段有 deliver:」推断。通过 loadJsonWorkflow 端到端验证。
...
import { loadJsonWorkflow } from "../src/core/workflow/registry";
...
describe("delivers 从 phases 派生（deriveDelivers）", () => {
  ...
  async function load(spec: Record<string, unknown>) {
    const dir = join(tmp, "wf");
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "workflow.json"), JSON.stringify(spec, null, 2));
    return loadJsonWorkflow(dir);
  }
  ...
  it("无顶层 delivers + deliver:artifacts 阶段 → 派生 artifacts", async () => {
    const w = await load({
      name: "wf",
      phases: [
        { name: "produce", prompt: "写到 ${DELIVERABLES}" },
        { name: "deliver", deliver: "artifacts" },
      ],
    });
      ...
      phases: [
        {
          parallel: {
            ...
            phases: [
              { name: "fe", prompt: "x" },
              { name: "deliver_fe", deliver: "artifacts" },
            ],
          },
        },
      ],
