    ...
    def test_full_lifecycle(self):
        ...
        for trigger, expected_status, desc in expected_flow:
            ...
            assert task['status'] == expected_status, \
                f'{desc}: 期望 {expected_status}，实际 {task["status"]}'
...
class TestDesignRejectAndRetry:
    ...
    def test_max_rejections_triggers_cancel(self):
        ...
        for i in range(10):
            ...
            if i < 9:
                transition(tid, 'review_reject',
                           extra_updates={'rejection_count': i + 1})
                transition(tid, 'retry_design')
            else:
                # 第 10 次：取消
                transition(tid, 'cancel',
                           note=f'方案评审驳回 10 次，已取消',
                           extra_updates={'rejection_count': 10})
...
class TestCodeRejectAndRework:
    ...
    def test_code_rejection_rework(self):
        ...
        # 代码审查驳回
        transition(tid, 'code_reject',
                   extra_updates={'code_rejection_count': 1, 'rejection_reason': '缺少单元测试'})
...
class TestCancelFromAnyPhase:
    ...
    @pytest.mark.parametrize('setup_triggers,phase_name', [
        ([], 'pending_design'),
        ...
        (['start_design', 'design_complete', 'start_review', 'review_pass',
          'start_dev', 'dev_complete'], 'code_reviewing'),
    ])
...
class TestMultipleTasksConcurrent:
    ...
    def test_independent_tasks(self):
        ...
        # 任务一快速完成
        for t in ['start_design', 'design_complete', 'start_review', 'review_pass',
                   'start_dev', 'dev_complete', 'code_pass']:
            transition(tid1, t)
...
class TestLogIntegrity:
    ...
    def test_every_transition_logged(self):
        tid = _register_task()
        triggers = ['start_design', 'design_complete', 'start_review',
                     'review_reject', 'retry_design', 'start_design',
                     'design_complete', 'start_review', 'review_pass',
                     'start_dev', 'dev_complete', 'code_pass']

        for t in triggers:
            transition(tid, t, extra_updates=(
                {'rejection_count': 1} if t == 'review_reject' else None
            ))
        ...
        for i in range(len(logs_asc) - 1):
            assert logs_asc[i]['to_status'] == logs_asc[i + 1]['from_status'], \
                f'日志链断裂：第{i}条 to={logs_asc[i]["to_status"]} != 第{i+1}条 from={logs_asc[i+1]["from_status"]}'
