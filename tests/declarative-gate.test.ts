 ...
 * P1 后：改用 loadJsonWorkflow + workflow.json（yaml 文件已删）。
 */
import { describe, it, expect, afterEach } from "bun:test";
import { mkdirSync, rmSync, writeFileSync, existsSync } from "fs";
import { join } from "path";
import { tmpdir } from "os";
import { loadJsonWorkflow, _clearRegistry } from "../src/core/workflow/registry";
...
function makeWf(spec: Record<string, unknown>, ts?: string): string {
...
describe("declarative 安全闸门", () => {
  it("declarative:true + workflow.ts 含 run_ 函数 → ts 被忽略（warn），工作流仍正常加载", async () => {
    const dir = makeWf(
      { name: "decl_ok_ts", declarative: true, phases: [{ name: "design", prompt: "做设计" }] },
  ...
  it("declarative:true + 纯 prompt phase（无 ts）→ 正常加载", async () => {
    const dir = makeWf(
      { name: "decl_prompt", declarative: true, phases: [{ name: "design", prompt: "做设计" }] },
    );
    const wf = await loadJsonWorkflow(dir);
  ...
  it("declarative:true + phase 既无 prompt 也无 gate → 报错（缺框架原语）", async () => {
    const dir = makeWf(
      { name: "decl_empty", declarative: true, phases: [{ name: "design", timeout: 600 }] },
    );
    await expect(loadJsonWorkflow(dir)).rejects.toThrow(/缺少框架原语|不能用 run_/);
  });
  ...
  it("非 declarative + run_ 函数 → 不受闸门影响（正常绑定）", async () => {
    const dir = makeWf(
      { name: "plain_ts", phases: [{ name: "design", timeout: 600 }] },
      "export async function run_design(_t: string): Promise<void> {}\n",
    );
    const wf = await loadJsonWorkflow(dir);
    expect(wf).not.toBeNull();
