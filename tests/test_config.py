...
class TestValidateConfig:
    ...
    def test_valid_config_no_errors(self):
        ...
        config = {
            "default_branch": "main",
            "timeouts": {"design": 600, "review": 300},
        }
    ...
    def test_nested_validation(self):
        """嵌套配置校验"""
        config = {"timeouts": {"design": "not_an_int"}}
        errors, warnings = validate_config(config)
        assert len(errors) == 1
        assert "timeouts.design" in errors[0]

    def test_nested_unknown_key(self):
        """嵌套未知配置项产生 warning"""
        config = {"timeouts": {"design": 600, "unknown_phase": 100}}
        errors, warnings = validate_config(config)
        assert errors == []
        assert len(warnings) == 1
        assert "timeouts.unknown_phase" in warnings[0]
