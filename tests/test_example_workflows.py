"""
示例工作流端到端测试：从 examples/ 目录磁盘加载，完成加载→校验→注册→转换表→状态流转全链路
"""

from __future__ import annotations

import importlib.util
import sys
from pathlib import Path
from types import SimpleNamespace

import pytest

from core.db import create_task, get_task
from core.registry import (
    WorkflowValidationError,
    _registry,
    build_transitions,
    register,
    validate_workflow,
)
from core.state_machine import transition

EXAMPLES_DIR = Path(__file__).parent.parent / "examples"


# ──────────────────────────────────────────────────────────
# 辅助函数
# ──────────────────────────────────────────────────────────


def _load_example_module(example_dir: Path):
    """从 examples/ 目录加载 workflow.py 模块"""
    wf_file = example_dir / "workflow.py"
    if not wf_file.exists():
        pytest.skip(f"workflow.py 不存在：{example_dir}")

    mod_name = f"test_example_{example_dir.name}"
    # 清理缓存模块
    if mod_name in sys.modules:
        del sys.modules[mod_name]

    spec = importlib.util.spec_from_file_location(mod_name, wf_file)
    if spec is None or spec.loader is None:
        pytest.skip(f"无法加载模块：{wf_file}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[mod_name] = mod
    spec.loader.exec_module(mod)
    return mod


def _get_example_dirs():
    """获取所有包含 workflow.py 的示例目录"""
    dirs = []
    for d in sorted(EXAMPLES_DIR.iterdir()):
        if d.is_dir() and (d / "workflow.py").exists():
            dirs.append(d)
    return dirs


def _register_task(task_id, workflow):
    """注册一个测试任务"""
    create_task(
        task_id=task_id,
        req_id=f"REQ-{task_id}",
        title=f"测试任务 {task_id}",
        project="test-project",
        repo_path="/tmp/test-repo",
        branch=f"feat/{task_id}",
        agents={},
        notify_target="",
        channel="log",
        workflow=workflow,
    )


@pytest.fixture
def _register_example(request):
    """注册示例工作流并在测试结束后清理"""
    example_dir = request.param
    mod = _load_example_module(example_dir)
    old_registry = dict(_registry)
    register(mod)
    yield mod
    _registry.clear()
    _registry.update(old_registry)


# ──────────────────────────────────────────────────────────
# 参数化示例目录列表
# ──────────────────────────────────────────────────────────

EXAMPLE_DIRS = _get_example_dirs()
EXAMPLE_IDS = [d.name for d in EXAMPLE_DIRS]


# ──────────────────────────────────────────────────────────
# 加载与校验测试
# ──────────────────────────────────────────────────────────


class TestExampleWorkflowsLoadAndValidate:
    """参数化测试：遍历所有 examples/*/workflow.py"""

    @pytest.mark.parametrize("example_dir", EXAMPLE_DIRS, ids=EXAMPLE_IDS)
    def test_load_from_disk(self, example_dir):
        """从磁盘加载 workflow.py 成功"""
        mod = _load_example_module(example_dir)
        assert hasattr(mod, "WORKFLOW"), f"{example_dir.name} 缺少 WORKFLOW 字典"
        assert isinstance(mod.WORKFLOW, dict)

    @pytest.mark.parametrize("example_dir", EXAMPLE_DIRS, ids=EXAMPLE_IDS)
    def test_validate_passes(self, example_dir):
        """validate_workflow 通过"""
        mod = _load_example_module(example_dir)
        warns = validate_workflow(mod.WORKFLOW)
        assert isinstance(warns, list)

    @pytest.mark.parametrize("example_dir", EXAMPLE_DIRS, ids=EXAMPLE_IDS)
    def test_prelaunch_passes(self, example_dir):
        """注册后校验通过：注册 → build_transitions → 非空"""
        mod = _load_example_module(example_dir)
        old_registry = dict(_registry)
        try:
            register(mod)
            wf_name = mod.WORKFLOW["name"]
            transitions = build_transitions(wf_name)
            assert isinstance(transitions, dict)
            assert len(transitions) > 0
            # initial_state 应在转换表中
            initial = mod.WORKFLOW["initial_state"]
            assert initial in transitions, f"initial_state '{initial}' 不在转换表中"
        finally:
            _registry.clear()
            _registry.update(old_registry)

    @pytest.mark.parametrize("example_dir", EXAMPLE_DIRS, ids=EXAMPLE_IDS)
    def test_build_transitions_nonempty(self, example_dir):
        """转换表非空"""
        mod = _load_example_module(example_dir)
        old_registry = dict(_registry)
        try:
            register(mod)
            wf_name = mod.WORKFLOW["name"]
            transitions = build_transitions(wf_name)
            assert len(transitions) > 0
        finally:
            _registry.clear()
            _registry.update(old_registry)

    @pytest.mark.parametrize("example_dir", EXAMPLE_DIRS, ids=EXAMPLE_IDS)
    def test_yaml_exists(self, example_dir):
        """如果有 workflow.yaml 则可以被 PyYAML 加载"""
        yaml_path = example_dir / "workflow.yaml"
        if not yaml_path.exists():
            pytest.skip(f"无 workflow.yaml：{example_dir.name}")
        import yaml

        with open(yaml_path, encoding="utf-8") as f:
            data = yaml.safe_load(f)
        assert isinstance(data, dict)
        assert "name" in data
        assert "phases" in data


# ──────────────────────────────────────────────────────────
# doc_gen 状态机测试
# ──────────────────────────────────────────────────────────


class TestDocGenStateMachine:
    """doc_gen 工作流状态流转测试（自动推导转换表）"""

    @pytest.fixture(autouse=True)
    def _setup_doc_gen(self):
        """注册 doc_gen 工作流"""
        doc_gen_dir = EXAMPLES_DIR / "doc_gen"
        mod = _load_example_module(doc_gen_dir)
        old_registry = dict(_registry)
        register(mod)
        yield
        _registry.clear()
        _registry.update(old_registry)

    def test_happy_path(self):
        """自动推导 → pending_generate → running_generate → ... → done"""
        _register_task("DG-001", "doc_gen")
        task = get_task("DG-001")
        assert task["status"] == "pending_generate"

        flow = [
            ("start_generate", "running_generate"),
            ("generate_complete", "pending_review_doc"),
            ("start_review_doc", "running_review_doc"),
            ("review_doc_pass", "doc_gen_done"),
        ]
        for trigger, expected in flow:
            _, to = transition("DG-001", trigger)
            assert to == expected

        task = get_task("DG-001")
        assert task["status"] == "doc_gen_done"

    def test_reject_cycle(self):
        """review_doc reject → retry generate → 再通过"""
        _register_task("DG-002", "doc_gen")

        # 第一轮：generate → review → reject
        for t in ["start_generate", "generate_complete", "start_review_doc"]:
            transition("DG-002", t)

        _, to = transition("DG-002", "review_doc_reject")
        assert to == "review_doc_rejected"

        _, to = transition("DG-002", "retry_generate")
        assert to == "pending_generate"

        # 第二轮：generate → review → pass
        for t in ["start_generate", "generate_complete", "start_review_doc"]:
            transition("DG-002", t)

        _, to = transition("DG-002", "review_doc_pass")
        assert to == "doc_gen_done"


# ──────────────────────────────────────────────────────────
# parallel_build 转换表测试
# ──────────────────────────────────────────────────────────


class TestParallelBuildTransitions:
    """parallel_build 工作流转换表测试"""

    @pytest.fixture(autouse=True)
    def _setup_parallel_build(self):
        """注册 parallel_build 工作流"""
        pb_dir = EXAMPLES_DIR / "parallel_build"
        mod = _load_example_module(pb_dir)
        old_registry = dict(_registry)
        register(mod)
        yield
        _registry.clear()
        _registry.update(old_registry)

    def test_fork_join_transitions(self):
        """转换表包含 pending_build → waiting_build、build_complete → ..."""
        transitions = build_transitions("parallel_build")
        assert "pending_build" in transitions
        assert "waiting_build" in transitions

        # pending_build → waiting_build（fork 点）
        pb_triggers = {t: d for t, d in transitions["pending_build"]}
        assert "start_build" in pb_triggers
        assert pb_triggers["start_build"] == "waiting_build"

        # waiting_build → pending_integration_test（join 点）
        wb_triggers = {t: d for t, d in transitions["waiting_build"]}
        assert "build_complete" in wb_triggers
        assert wb_triggers["build_complete"] == "pending_integration_test"

    def test_sub_phase_transitions(self):
        """子阶段 pending → running → done"""
        transitions = build_transitions("parallel_build")

        # build_frontend 子阶段
        assert "pending_build_frontend" in transitions
        assert "running_build_frontend" in transitions
        bf_triggers = {t: d for t, d in transitions["pending_build_frontend"]}
        assert "start_build_frontend" in bf_triggers
        assert bf_triggers["start_build_frontend"] == "running_build_frontend"

        bf_run_triggers = {t: d for t, d in transitions["running_build_frontend"]}
        assert "build_frontend_complete" in bf_run_triggers
        assert bf_run_triggers["build_frontend_complete"] == "build_frontend_done"

        # build_backend 子阶段
        assert "pending_build_backend" in transitions
        assert "running_build_backend" in transitions
        bb_triggers = {t: d for t, d in transitions["pending_build_backend"]}
        assert "start_build_backend" in bb_triggers

    def test_hooks_defined(self):
        """hooks 已正确定义"""
        from core.registry import get_workflow

        wf = get_workflow("parallel_build")
        assert "hooks" in wf
        assert "before_phase" in wf["hooks"]
        assert "after_phase" in wf["hooks"]
        assert callable(wf["hooks"]["before_phase"])
        assert callable(wf["hooks"]["after_phase"])

    def test_happy_path_state_flow(self):
        """完整路径：prepare → build → integration_test → done"""
        _register_task("PB-001", "parallel_build")
        task = get_task("PB-001")
        assert task["status"] == "pending_prepare"

        flow = [
            ("start_prepare", "running_prepare"),
            ("prepare_complete", "pending_build"),
            ("start_build", "waiting_build"),
            ("build_complete", "pending_integration_test"),
            ("start_integration_test", "running_integration_test"),
            ("integration_test_complete", "build_done"),
        ]
        for trigger, expected in flow:
            _, to = transition("PB-001", trigger)
            assert to == expected


# ──────────────────────────────────────────────────────────
# data_pipeline 状态机测试
# ──────────────────────────────────────────────────────────


class TestDataPipelineStateMachine:
    """data_pipeline 工作流状态流转测试"""

    @pytest.fixture(autouse=True)
    def _setup_data_pipeline(self):
        """注册 data_pipeline 工作流"""
        dp_dir = EXAMPLES_DIR / "data_pipeline"
        mod = _load_example_module(dp_dir)
        old_registry = dict(_registry)
        register(mod)
        yield
        _registry.clear()
        _registry.update(old_registry)

    def test_happy_path(self):
        """extract → validate → transform → load → completed"""
        _register_task("DP-001", "data_pipeline")
        task = get_task("DP-001")
        assert task["status"] == "pending_extract"

        flow = [
            ("start_extract", "running_extract"),
            ("extract_complete", "pending_validate_data"),
            ("start_validate_data", "running_validate_data"),
            ("validate_data_complete", "pending_transform"),
            ("start_transform", "running_transform"),
            ("transform_complete", "pending_load"),
            ("start_load", "running_load"),
            ("load_complete", "completed"),
        ]
        for trigger, expected in flow:
            _, to = transition("DP-001", trigger)
            assert to == expected

        task = get_task("DP-001")
        assert task["status"] == "completed"

    def test_forward_jump(self):
        """validate_skip 跳转到 load"""
        _register_task("DP-002", "data_pipeline")

        # extract → validate_data → skip to load
        for t in ["start_extract", "extract_complete", "start_validate_data"]:
            transition("DP-002", t)

        _, to = transition("DP-002", "validate_skip")
        assert to == "pending_load"

        # 继续 load → completed
        transition("DP-002", "start_load")
        _, to = transition("DP-002", "load_complete")
        assert to == "completed"

    def test_reject_back_to_extract(self):
        """transform reject → extract"""
        _register_task("DP-003", "data_pipeline")

        # extract → validate → transform → reject
        for t in [
            "start_extract",
            "extract_complete",
            "start_validate_data",
            "validate_data_complete",
            "start_transform",
        ]:
            transition("DP-003", t)

        _, to = transition("DP-003", "transform_reject")
        assert to == "transform_rejected"

        _, to = transition("DP-003", "retry_extract")
        assert to == "pending_extract"

    def test_multiple_terminal_states(self):
        """多终态：completed_partial"""
        _register_task("DP-004", "data_pipeline")

        for t in [
            "start_extract",
            "extract_complete",
            "start_validate_data",
            "validate_data_complete",
            "start_transform",
            "transform_complete",
            "start_load",
        ]:
            transition("DP-004", t)

        _, to = transition("DP-004", "load_partial")
        assert to == "completed_partial"

    def test_retry_policy_defined(self):
        """retry_policy 已正确定义"""
        from core.registry import get_workflow

        wf = get_workflow("data_pipeline")
        assert "retry_policy" in wf
        assert wf["retry_policy"]["max_retries"] == 5
        assert wf["retry_policy"]["backoff"] == "exponential"
        assert wf["retry_policy"]["delay"] == 30
