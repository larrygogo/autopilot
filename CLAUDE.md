...
内核掌握"能做什么"的完整集合。一切上层 UI 只是它的不同 affordance，不应私藏业务概念或绕过内核。内核变化先在 core / daemon / agents / 协议上落地，三套 UI 跟上。

**三套客户端 — 不互为子集，是面向不同心智的不同 affordance**

| 客户端 | 面向人群 | 设计重心 |
|--------|---------|---------|
| **Web** | **决策者**（无关技术/非技术，凡是要"批准 / 驳回 / 调整方向"的人） | 可视化决策台。强引导、强故事、强 CTA。隐藏底层 trigger 名（`review_pass` → "通过审批"），用业务语言。状态完整性优先，操作确认到位。 |
| **TUI** | **本地执行者/观察者**（开发者实时盯进度、看日志） | 终端原生、键盘流、能塞进 tmux/IDE。替代频繁刷 web，主打"低成本持续观察"。 |
| **CLI** | **技术专家 + 自动化**（CI/CD、脚本、远程 ssh） | 与内核 1:1 暴露面。命令完整、参数显式、退出码可用。不隐藏底层概念（保留 `--trigger review_pass` 这类原始语义）。 |

**落地原则**

- 同一功能在 Web / TUI / CLI 都应可达（来自同一份内核语义），但**暴露面不同**：Web 可折叠成"下一步主 CTA"，CLI 必须显式
- 三套 UI 各自的语言习惯（业务标签 vs trigger 名 vs 命令参数）由客户端层翻译，**内核不为某个 UI 妥协命名**
- 添加新功能时先问："决策者要不要点这个按钮？观察者要不要看这条信息？自动化要不要调这个？" — 三个问题的答案决定它在哪几套 UI 上长什么样
