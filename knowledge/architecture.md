...
┌─────────────────────────────────────────────────────────────┐
│                        CLI 入口层                            │
│   start_task.py    run_phase.py    cancel_task.py   watcher │
└──────┬──────────────────┬──────────────┬──────────────┬─────┘
       │                  │              │              │
┌──────▼──────────────────▼──────────────▼──────────────▼─────┐
│                      Runner（执行引擎）                       │
...
│  阶段查询     │  │  转换表验证       │  │  跨平台文件锁    │
│  转换表生成   │  │  日志记录         │  │  Claude CLI      │
└──────┬───────┘  └───────┬──────────┘  └──────────────────┘
       │                  │
┌──────▼──────────────────▼──────────────────────────────────┐
│                     DB（持久化层）                           │
│  SQLite: tasks 表 + task_logs 表                           │
│  config.yaml 加载                                          │
└────────────────────────────────────────────────────────────┘
       │
┌──────▼────────────────────────────────────────────────────┐
...
- 启动时自动扫描 `src/dev_workflow/workflows/` 目录
- 使用 `pkgutil.iter_modules` 发现所有含 `WORKFLOW` 字典的模块
- 提供查询接口：获取工作流定义、阶段函数、转换表
- 支持从 `WORKFLOW['phases']` 自动生成状态转换表
...
| `get_available_triggers(task_id)` | 列出当前状态可用触发器 |
...
| 通知 | `notify(task, message)` | 通过 OpenClaw 发送通知 |
| AI | `run_claude(prompt, repo_path)` | 调用 Claude CLI |
| 需求 | `fetch_req(req_id)` | 从 ReqGenie 获取需求 |
...
- 配置文件加载（优先级：环境变量 > 当前目录 > 用户目录 > 项目根目录）
...
```
用户执行: python3 bin/start_task.py <req_id> --project my-project
    │
    ▼
...
[3] execute_phase(task_id, 'review')
    ...
    │
    ▼ (新进程，仅通过时)
[4] execute_phase(task_id, 'dev')
    ├── 切换分支、调用 Claude 实现代码、提交变更
    ├── transition ──[dev_complete]──→ code_reviewing
    └── run_in_background(task_id, 'code_review')
    │
    ▼
[5] execute_phase(task_id, 'code_review')
    ├── 获取 git diff、调用 Claude 审查
    ├── 通过: transition ──[code_pass]──→ pr_submitting
    │   └── run_in_background(task_id, 'pr')
    └── 驳回: 回退到 dev 阶段重试
    │
    ▼
[6] execute_phase(task_id, 'pr')
    ├── 推送分支、生成 PR 描述、创建 GitHub PR
    └── 最终状态: pr_submitted ✓
```
...
**Watcher 作为保底**：Push 偶尔可能失败（进程启动失败），Watcher 每 5 分钟扫描一次卡死任务并恢复，补偿了 Push 的不可靠性。
