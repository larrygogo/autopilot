...
- **run 日志 30 天保留（P2 长跑健康）**：新增 retention 第三轨 `pruneTerminalRequirementRunLogs`——**终态需求超 `sandbox_retention.days`（默认 30）天后清其 run 目录**（phase 日志 / agent-calls / manifest；`runtime/requirements/<reqId>/runs/`）。此前 retention 只清代码目录（workspace/codebase），run 日志文件随「run 多历史」设计单调增长、无清理。**age 以需求终态时间（updated_at）为准、不用 run 目录 mtime**（code 轨删 workspace 会刷新 mtime → 用 mtime 会漏清）；**tasks DB 行保留**（run 多历史账本，需求页仍列出 run，过窗口后无文件级日志详情，是「保留历史 vs 磁盘」的 30 天权衡）；retention.ts 保持 DB-free，终态时间由 watcher `requirementTerminalAt` 注入；legacy `runtime/tasks/` 不在此轨（冻结存量自然老化）。
